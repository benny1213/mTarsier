...
const AGENT_SKILLS_SVG = `<svg fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 2.25 4.5 6.58v8.66l7.5 4.33 7.5-4.33V6.58L12 2.25Zm0 2.02 4.35 2.51L12 9.29 7.65 6.78 12 4.27Zm-5.75 4.02L11 11.03v5.91l-4.75-2.74V8.29Zm6.75 8.65v-5.91l4.75-2.74v5.91L13 16.94Z"/><path d="M4 18.25a1 1 0 0 1 1 1v1.5h1.5a1 1 0 1 1 0 2H4a1 1 0 0 1-1-1v-2.5a1 1 0 0 1 1-1Zm16 0a1 1 0 0 1 1 1v2.5a1 1 0 0 1-1 1h-2.5a1 1 0 1 1 0-2H19v-1.5a1 1 0 0 1 1-1Z"/></svg>`;
...
const COLORED_LOGO_IDS = new Set(["cursor", "mcpporter", "opencode", "trae", "trae-cn"]);

function getLogoSvg(clientId: string, isDark: boolean): string | null {
  void isDark;
  if (clientId === "claude-code") return CLAUDE_CODE_SVG;
  if (clientId === "agent-skills-shared") return AGENT_SKILLS_SVG;
...
export function ClientLogo({ clientId, clientName, size = 22 }: ClientLogoProps) {
  const isDark = useThemeStore((s) => s.theme.monaco.base === "vs-dark");
  const svg = getLogoSvg(clientId, isDark);
  const isColoredLogo = COLORED_LOGO_IDS.has(clientId);
  const filterClass = isColoredLogo ? "" : isDark ? "brightness-0 invert" : "brightness-0";
